...
void loop() {
  ...
  if (current_time - prev[2] > 1) {
    ...
    //cahnge offset1 to a value lower than the resting value of the accelo
  ...
  if (prev_follow1 < threshold1 && follow1 > threshold1) {
    delta_ped_time = millis() - ped_time;
    ped_time = millis();
    drum1.noteOn();
  }
  ...
  final_output_level = map(A2, 0, 1023.0, 0, 100) / 100.0; //if we don't want it to go over a certain level we can just cahnge the last value in map

...
int note_on_length = 5; //How long to have the envelope gate on for each note
int seq_rate;
int seq_index;
int prev_seq_index;
...
void loop() {
  ...
  filter1_freq = wave1_freq * (((analogRead(A4) / 1023.0) * 3.0) + .75);
  ...
  if (current_time - note_off_time > note_on_length && note_off_time > 0) {
  ...
  if (current_time - prev[0] > 33) { //33 millis is about 30Hz, aka fps
    ...
    for ( int x_count = 0; x_count < 8; x_count++) {
      for ( int y_count = 0; y_count < 8; y_count++) {
        ...
        int drum_led = drum_seq[xy_count];

        if (drum_led == 1) {
          float hue = .3; //each row will be a diff color
          set_pixel(xy_count, hue , .9, 1);
        }
        if (drum_led == 2) {
          float hue = .5; //each row will be a diff color
          set_pixel(xy_count, hue , .9, 1);
        }

//Play a defined sequence of notes when you hold down the first button
//Two oscilltaors play the same notes but one is detuned using the bottom left pot
//Screen displays the note being played 
/*
  pots:
...
void loop() {
  ...
  if (debouncer1.fell()) {
    arp_enable = 1;
    prev[2] = 0;  //set to 0 so the sequence satrs immediatley when pressing the button
    arp_index = 0; // and so it starts on the first step
  ...
  //note_offset = map(analogRead(A3), 0, 1023, 0, 60);
  note_offset = 10;

  float detune = ((analogRead(A3) / 1023.0)) + 1.0; // 1.0-2.0 where 1 will play the exat same note and 2 will play it an octave higher
  ...
  if (current_time - prev[2] > arp_rate) {
    ...
    if (arp_enable == 1) {
      //use the pot so select which array to use
      // note select is the offset, otherwise it would jsut play very low notes

      if (scale_select == 0) {
        //add the arp index to the offset and use that to select a p,lace in the major array
        // then use that value to select a fewquecy from chromatic
        note_select = major[arp_index + note_offset]; //What note we'll actually play
        wave1_freq = chromatic[note_select];
      }
      if (scale_select == 1) {
        note_select = minor[arp_index + note_offset];
        wave1_freq = chromatic[note_select];
      }
      if (scale_select == 2) {
        note_select = diminished[arp_index + note_offset];
        wave1_freq = chromatic[note_select];
      }
      if (scale_select == 3) {
        note_select = arp_index + note_offset;
        wave1_freq = chromatic[note_select];
      }
  ...
  if (current_time - note_off_timer > note_length && note_off_timer > 0) {
    //turn notes off after a set amount of time
    envelope1.noteOff();
    envelope2.noteOff();
    note_off_timer = 0;
  }
  ...
  if (current_time - prev[0] > 33) { //33 millis is about 30Hz, aka fps
    ...
    for ( int x_count = 0; x_count < 8; x_count++) {
      for ( int y_count = 0; y_count < 8; y_count++) {
        ...
        //Display the note being played on the screen 
        if (arp_enable == 1) {
          if (note_select > 63) {
            note_select -= 63; //Wrap it around and start at the top again
          }
          if (xy_count == note_select) {
            float hue = xy_count / 63.0; //each row will be a different color

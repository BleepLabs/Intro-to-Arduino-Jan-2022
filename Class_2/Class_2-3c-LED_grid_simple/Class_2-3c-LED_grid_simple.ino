  ...
  if (current_time - prev[0] > 33) { //33 millis is about 30Hz, aka fps
    ...
    for ( int x_count = 0; x_count < 8; x_count++) {
      for ( int y_count = 0; y_count < 8; y_count++) {
        ...
        set_pixel(xy_count, rainbow1 , 1, 0); // turn everything off. otherwise the last "frame" swill still show

        /*
                int j1 = xy_sel + 1;
                if (xy_count == j1 && x_count != 7) {
                  set_hue = .4;
                  set_pixel(j1, set_hue , 0, 1);
                }
        */
      }
    }
    ...
    set_pixel(xy_sel, .4 , .9, 1);
    set_pixel(xy_sel + 1, .2 , .9, 1);
    set_pixel(xy_sel - 1, .2 , .9, 1);
    set_pixel(xy_sel + 8, .3 , .9, 1);
    set_pixel(xy_sel - 8, .3 , .9, 1);

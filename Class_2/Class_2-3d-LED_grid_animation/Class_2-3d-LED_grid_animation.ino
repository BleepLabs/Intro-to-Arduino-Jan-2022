  ...
  if (current_time - prev[0] > 33) { //33 millis is about 30Hz, aka fps
    ...
    for ( int x_count = 0; x_count < 8; x_count++) {
      for ( int y_count = 0; y_count < 8; y_count++) {
        ...
        rainbow1 = (xy_count / 64.0);
        set_pixel(xy_count, rainbow1 , 1, .25); // turn everything off. otherwise the last "frame" swill still show
      }
    }

    float move1 = lfo1 + (lfo2*8);
    set_pixel(move1, .4 , .9, 1);
    set_pixel(move1 + 1, .2 , .9, 1);
    set_pixel(move1 - 1, .2 , .9, 1);
    set_pixel(move1 + 8, .3 , .9, 1);
    set_pixel(move1 - 8, .3 , .9, 1);
    
    set_pixel(xy_sel, .3 , 0, .5);
